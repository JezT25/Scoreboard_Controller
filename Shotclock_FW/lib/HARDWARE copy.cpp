...
volatile int HARDWARE_class::S1      = 99;
volatile int HARDWARE_class::S2      = 99;
volatile int HARDWARE_class::S3      = 99;
...
void IRAM_ATTR HARDWARE_class::DisplayLED() {
    ...
    if(Power_Flag == POWER_ON)
    {
        ...
        if(CurrentSegment == TENS_SEGMENT) CurrentSegment = ONES_SEGMENT;
        if(CurrentSegment == ONES_SEGMENT) CurrentSegment = TENS_SEGMENT;
    }
}
