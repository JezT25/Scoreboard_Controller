...
void IRAM_ATTR HARDWARE_class::DisplayLED() {
    ...
    else
    {
        ...
        else if(CurrentSegment == PERIOD_SEGMENT)
        {
            ...
            digitalWrite(D0, (pattern >> 0) & 0x01);
            digitalWrite(D1, (pattern >> 1) & 0x01);
            digitalWrite(D2, (pattern >> 2) & 0x01);
            digitalWrite(D3, (pattern >> 3) & 0x01);
            digitalWrite(D4, (pattern >> 4) & 0x01);
            digitalWrite(D5, (pattern >> 5) & 0x01);
            digitalWrite(D6, (pattern >> 6) & 0x01);
            digitalWrite(D7, (pattern >> 7) & 0x01);
        }
        else if(CurrentSegment == POS_SEGMENT)
        {
            if(Clock_Flag == HIGH && pPosession == Posession) Posession = NO_POSESSION;
            else if(Clock_Flag == HIGH && pPosession != Posession) pPosession = 10;
            ...
            digitalWrite(D4, (Clock_Flag == HIGH && pHome_Score == Home_Score) ? 0 : Home_Score > 99);
            digitalWrite(D5, (Clock_Flag == HIGH && pAway_Score == Away_Score) ? 0 : Away_Score > 99);
            digitalWrite(D6, 0);
            digitalWrite(D7, 0);
        }
